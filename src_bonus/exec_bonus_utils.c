...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/13 09:43:32 by gcosta-m          #+#    #+#             */
/*   Updated: 2025/01/13 09:43:35 by gcosta-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	open_output_file(t_fd *fd, char **av, t_data *pipex)
{
	...
	{
		fd->output_file = open(outfile_path, O_WRONLY | O_TRUNC | O_CREAT,
				0777);
		if (fd->output_file < 0)
		{
	...
	{
		fd->output_file = open(outfile_path, O_WRONLY | O_APPEND | O_CREAT,
				0777);
		if (fd->output_file < 0)
		{
			ft_printf("%s: ", av[pipex->ac - 1]);
