...
MU_TEST(function_should_run_comand_ls_l_wc_l)
{
    ...
    exec_cmd("../pipex_bonus ../infile \"ls -l\" \"wc -l\" ../outfile");
...
MU_TEST(funtion_should_run_command_grep_a1_wc_w)
{
	...
	exec_cmd("../pipex_bonus infile \"grep a1\" \"wc -w\" ../outfile2");
...
MU_TEST(funtion_should_run_command_cat_ls_l)
{
	...
	exec_cmd("../pipex_bonus infile \"cat\" \"ls -l\" ../outfile3");
...
MU_TEST(funtion_should_run_command_cat_wc)
{
	...
	exec_cmd("../pipex_bonus infile \"cat\" \"wc\" ../outfile4");
...
MU_TEST(funtion_should_run_command_echo_tr)
{
	...
	exec_cmd("../pipex_bonus infile \"echo CAIU_NA_VILA_O_PEIXE_FUZILA\" \"tr -d _\" ../outfile5");
...
MU_TEST(funtion_should_create_outfile_and_run_second_command)
{
	...
	exec_cmd("../pipex_bonus ../file_x \"cat\" \"ls\" ../outfile6");
...
MU_TEST(funtion_should_run_command_echo_tr_space)
{
	...
	exec_cmd("../pipex_bonus ../infile \"echo \'h e l l o\'\" \"tr -d ' '\" ../outfile7");
...
MU_TEST(function_should_try_run_command_without_x_permission)
{
    ...
    exec_cmd("../pipex_bonus ../infile \"grep pipex\" \"/dev/null\" ../outfile8");
...
MU_TEST(function_should_try_run_command_with_path)
{
    ...
    exec_cmd("../pipex_bonus ../infile \"////////usr/bin/grep 1\" \"wc -l\" ../outfile9");
...
MU_TEST(function_should_run_5_commands)
{
    ...
    printf("\n------------------------\n");
    printf(" TEST 10 - REAL_BONUS_PART 5 commands: ");
    printf("\n------------------------\n");

    exec_cmd("< ../infile2 cat | head -n 5 | grep x | tr -d ' ' | rev  > ../outfile_expected10");
    exec_cmd("../pipex_bonus ../infile \"cat\" \"head -n 5\" \"grep x\" \"tr -d ' '\" \"rev\" ../outfile10");

    diff_result = exec_cmd("LC_ALL=C diff -s ../outfile10 ../outfile_expected10");
    ft_printf("%s", diff_result);
    outfile = exec_cmd("cat ../outfile10");
    ft_printf("Outfile: %s\n", outfile);
    mu_assert_string_eq(expected, diff_result);
    free(diff_result);
	free(outfile);
...
MU_TEST(function_should_overwrite_output)
{
    ...
    printf("\n------------------------\n");
    printf(" TEST 10.2 - REAL_BONUS_PART 5 commands: ");
    printf("\n------------------------\n");
...
MU_TEST_SUITE(test_suite)
{
    ...
    MU_RUN_TEST(function_should_run_5_commands);
    MU_RUN_TEST(function_should_overwrite_output);
