...
void	get_comands_args(char **av, t_data *pipex)
{
	...
	pipex->cmd_qtd = pipex->ac - 3;
	pipex->comands_args = ft_calloc((pipex->cmd_qtd + 1), sizeof(char **));
	if (!pipex->comands_args)
		ft_handle_errors("cmd calloc failed", pipex, NULL, 1);
	pipex->cmd_args_qtd = ft_calloc((pipex->cmd_qtd + 1), sizeof(char **));
	if (!pipex->cmd_args_qtd)
	{
	...
	{
		pipex->cmd_args_qtd[i] = ft_count_args(av[i + 2], ' ');
		pipex->comands_args[i] = ft_split_args(av[i + 2], ' ');
		if (!pipex->comands_args[i])
		{
			free(pipex->comands_args);
			free(pipex->cmd_args_qtd);
